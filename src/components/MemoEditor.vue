...
export default class MemoEditor extends Vue {
    ...
    saveNew(){
        // 1. 验证 memo 数据是否合格
        if( this.memo && this.memo.categoryId > -1 && this.memo.title.trim() !== "" && this.memo.content.trim() !== "" ){
            // 判定是新增 还是 修改 
            if( this.memo.id === -1 ){
              // 2. 将验证合格的 neno 对象添加在 memoList 数组，保存到 localStorage 中
